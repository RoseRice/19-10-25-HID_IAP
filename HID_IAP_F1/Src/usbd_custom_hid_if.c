...
USB_RCV_STU RcvUSB; // USB接收数据/状态
USB_SND_STU SndUSB; // USB发送数据/状态
...
static int8_t CUSTOM_HID_OutEvent_FS(uint8_t event_idx, uint8_t state)
{
  ...
  // 缓存接收数据 （ inote 可考虑用循环队列）
  memset(RcvUSB.buf, 0x00, sizeof(RcvUSB.buf)); // inote 保险起见
  RcvUSB.hasData = TRUE_t;
  memcpy(RcvUSB.buf, hhid->Report_buf, sizeof(RcvUSB.buf));
